...
public class Snowflake {
     ...
     * millisecond). its value will be increased by 1 in the same specified period and then reset to
     * 0 for next period.
     *
    ...
    public long[] parseId(long id) {
        ...
        arr[2] =
                (id & diode(unusedBits + timestampBits + datacenterIdBits, workerIdBits))
                        >> workerIdShift;
        arr[3] =
                (id
                        & diode(
                                unusedBits + timestampBits + datacenterIdBits + workerIdBits,
                                sequenceBits));
        return arr;
    }

    /**
     * extract and display time stamp, datacenterId, workerId and sequence number information from
     * the given id in humanization format
     *
